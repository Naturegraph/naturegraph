...
name: pipeline-dev
description: >
  Pipeline de dev Naturegraph G0 -> G10 : portes de validation sequentielles, un
  ROLE par porte (PM, Dev, Refactor, QA, Security, DBA, Doc, UX, Integration,
  Release Manager, Observabilite). A utiliser pour CHAQUE tache (feature, fix,
  refactor, migration), du cadrage a la prod. Trigger : "nouvelle tache",
  "developper", "feature", "fix", "on avance", "process dev", "les portes",
  "G0/G1...". Complete docs/devops/PIPELINE_DEV.md et le skill release-prod (G9).
---

# Pipeline de dev Naturegraph : G0 -> G10 (une porte, un role)
...
> La release elle-meme (G8/G9) suit le skill **release-prod** (squash, audit residus,
> rollback pret). Les checklists eco-conception/a11y detaillees : `GUIDELINES.md`.

## Comment jouer les roles

A chaque porte, adopter la **lentille du role** : on ne code pas en pensant "PM",
on ne valide pas la securite en pensant "Dev". Enoncer le verdict de la porte
(GO / NO-GO / N/A) avant de continuer. Pour une reponse produit, garder aussi la
revue multi-roles (PM -> UX -> Dev -> UI -> Lead Product Designer).
...
- Perimetre : front / back / DB / chemin rendu-loader-auth (ce dernier = vigilance mobile G7).
- Touche-t-on au **schema** ? si oui -> G5 obligatoire.
- Decoupage en etapes ; **hors perimetre** note.
...
- Commentaires du **pourquoi** au fur et a mesure. Pas de tiret cadratin/demi-cadratin.
...
- Eco-conception : JS < 300 KB gzip, page < 500 KB, LCP < 2,5 s ; images WebP/AVIF
  - lazy + dimensions ; CSS plutot que JS ; `prefers-reduced-motion`.
- Pas de dependance JS ajoutee sans justification.
- **GO** : code propre, budgets respectes.

## G3 · Qualite — QA · « Tout est vert ? »

- `npm run build` OK · `npm test` 100 % · `npx eslint .` 0 erreur · typecheck OK.
- **GO** : build + tests + lint verts. **NO-GO** -> retour G1/G2.

## G4 · Securite & donnees — Security · « Rien de rouge ? »

- Nouvelle table -> **RLS + policies** ; RPC `SECURITY DEFINER` volontaire et scopee.
...
## G5 · Base de donnees — DBA · « Migration testee sur le dev ? » (si schema)
...
- **GO** : migration testee dev, docs backend alignees. **Application prod = G9.** Sinon **N/A**.

## G6 · Docs & commentaires — Doc · « Code et docs disent la meme chose ? »
...
## G7 · Preview & validation UX — UX + Nicolas · « Valide mobile + desktop ? »

- Preview develop (base DEV) ouverte ; **console sans erreur**.
- Verifie **mobile ET desktop**, dark mode si pertinent.
...
- A11y : contraste >= 4,5:1, clavier, focus visible, skip link, alt.
- **GO** : Nicolas valide (obligatoire pour le chemin de rendu ; recommande sinon).

## G8 · Integration staging — Integration · « Staging stable ? »
...
## G9 · Release en prod — Release Manager · « Nicolas a dit OK ? »
...
- 2 release notes (technique + user-friendly) **soumises a Nicolas** (date, heure, force-logout ?, notif ?).
- **OK explicite de Nicolas** obtenu.
- Version bumpee + note dans `docs/devops/releases/`.
- PR `staging -> main` ; **CI verte** (Lint/Test/Build, CodeQL, Vercel) ; **merge SQUASH** (jamais `--admin`).
- **Migrations appliquees sur la PROD** (celles validees en G5) ; **back-merge `main -> develop`** ; tag `vX.Y.Z`.
- Notif in-app / force-logout uniquement si valides par Nicolas.
- **GO** : prod deployee sur `naturegraph.ca`.

## G10 · Suivi post-prod — Observabilite · « Prod saine 1 h apres ? »

- `naturegraph.ca` repond 200 ; bundle prod = base **prod** (`hrxg…`, jamais `nkgd…`).
- Surveillance **Sentry 30-60 min** post-deploy (aucune nouvelle issue).
- Advisors Supabase inchanges.
...
G3 Qualite        [ GO / NO-GO ]  build/test/lint : …
G4 Securite       [ GO / NO-GO ]  npm audit : … · RLS : …
G5 DB (migration) [ GO / N/A   ]  <fichier> teste dev : …
G6 Docs           [ GO / NO-GO ]  docs maj : …
...
- **G9 ne demarre jamais sans le OK explicite de Nicolas.**
- Chemin rendu/loader/auth = **validation mobile reelle de Nicolas** obligatoire (G7).
