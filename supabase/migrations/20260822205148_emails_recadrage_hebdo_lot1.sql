...
-- emails compte/securite/activation restent inchanges) :
--   - E7 social digest (reactions/commentaires) : quotidien -> DIMANCHE 18h
--   - E6 jalons especes                          : quotidien -> DIMANCHE 10h
...
begin
  ...
  -- E6 jalons especes -> dimanche 10h
  select jobid into j from cron.job where jobname = 'daily_species_milestones';
  if j is not null then perform cron.alter_job(j, schedule := '0 10 * * 0'); end if;
