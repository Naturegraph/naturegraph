...
function InstagramIcon() {
  return (
    <svg
      ...
      aria-hidden="true"
      focusable="false"
      className="text-[var(--color-text-white)] shrink-0"
    >
...
export function AuthHeroPhoto() {
  ...
  return (
    <div className="hidden lg:flex relative h-full shrink-0 w-[512px]">
      ...
      {/* Credit photographe (si renseigne) ou tagline de marque.
          Voile a 0.6 (et non 0.32) : le badge peut tomber sur une zone CLAIRE
          de la photo (tige, ciel), ou un voile faible laissait le texte blanc
          se delaver. A 0.6, le blanc #FFFDF8 garde >= 4,5:1 sur n'importe quelle
          photo. Texte force en blanc (valeur arbitraire = token non basculant),
          car il est sur media, pas sur une surface thematique. */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-lg bg-[rgba(12,12,20,0.6)] whitespace-nowrap">
        {photographerName ? (
          <div className="flex flex-col items-center gap-1">
            <div className="flex gap-2 items-center">
              <InstagramIcon />
              <p className="italic text-[var(--color-text-white)] text-[12px] tracking-wide">
                Crédit photo
              </p>
            </div>
            {instagramUrl ? (
              <a
                ...
                className="font-bold text-[var(--color-text-white)] text-sm underline hover:opacity-80 transition-opacity"
              >
                {photographerName}
              </a>
            ) : (
              <span className="font-bold text-[var(--color-text-white)] text-sm">
                {photographerName}
              </span>
            )}
          </div>
        ) : (
          <p className="text-[var(--color-text-white)] text-sm font-semibold tracking-wide italic">
            {tagline}
          </p>
        )}
      </div>
