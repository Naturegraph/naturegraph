...
/** Element minimal attendu : on ne depend que de la date de publication. */
...
export function buildFeedTimeline<T extends TimelineItem>(
  ...
  // Y a-t-il au moins une nouvelle observation ? Sinon, pas de frontiere "deja
  // vu" (tout est deja vu, ou premiere visite).
  const hasNew = lastVisitISO != null && posts.some((p) => p.created_at > lastVisitISO)
